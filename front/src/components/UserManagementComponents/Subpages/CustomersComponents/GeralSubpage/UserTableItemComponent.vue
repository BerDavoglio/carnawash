...
<template>
  <tr>
    ...
    <td className="text-[#3F3F44] font-light py-[24px] text-center">{{ this.obj.email }}</td>
    ...
    cursor-pointer underline underline-offset-1"
        @click="this.$emit('showC', [true, this.obj.id]);">
      View History
    </td>
    <td className="py-[24px] text-center">
      ...
      <span className="cursor-pointer"
            @click="deleteClient(this.obj.id)">
...
export default {
  ...
  methods: {
    editCs() {
      this.$emit('editCs', [true, this.obj]);
    },
