<!-- eslint-disable -->
<template>
  <div class="photo-data-component">
    <div className="h-[186px] w-[744px] p-[24px] mt-[40px]
    shadow-[4px_6px_40px_0px_#00000020] border-1 border-[#EBF0ED]">
      <div className="flex flex-row justify-between">
        <div className="flex flex-row">
          <div className="h-[128px] w-[128px]
          border-4 border-[#107EC4] border-solid rounded-full"></div>
          <div className="my-auto ml-8">
            <div className="text-[16px] font-semibold text-left">Washer A Name</div>
            <div className="text-[16px] font-normal text-left">1234 Washes</div>
          </div>
        </div>
        <div className="flex flex-col justify-between">
          <div className="flex flex-row text-right">
            <div className="text-[16px] font-semibold mr-2">
              Treined Washer
            </div>
            <div className="mr-2">
              <v-icon name="ri-shield-star-fill"
                      fill="#EDBD3A"
                      scale="2" />
            </div>
            <div className="text-[16px] font-semibold mr-2">
              4.8
            </div>
            <div className="mr-2">
              <v-icon name="bi-star-fill"
                      fill="#EDBD3A" />
            </div>
          </div>
          <div className="text-right text-[#EDBD3A] cursor-pointer">
            <span className="mr-2">
              <v-icon name="io-repeat-outline"
                      style="color: #EDBD3A;" />
            </span>Change Profile Picture
          </div>
        </div>
      </div>
    </div>
  </div>
</template>

<script>
export default {
  name: 'PhotoDataComponent',
};
</script>
