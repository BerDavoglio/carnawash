        ...
        <div className="text-left text-[16px] font-normal text-[#3F3F44]">
          ...
          <div className="ml-auto">
            <div className="w-full h-fit rounded-lg
              bg-[#F8F8F8] border-1 border-solid border-[#EBF0ED]">
              <input v-model="size.additional_information"
...
export default {
  ...
  methods: {
    async deleteOne(id) {
      ...
      await servicesStore.deleteSize(id);
    },
    async editAll() {
      const servicesStore = useServicesStore();

      this.listSize.forEach(async (obj) => {
        await servicesStore.editSize(obj.id, obj);
      });
    },
  },
  async beforeMount() {
    const servicesStore = useServicesStore();
    this.listSize = servicesStore.getSize;
  },
};
</script>
